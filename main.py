...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # add in skip connections from the first pooling layer
    skip = tf.layers.conv2d_transpose(vgg_layer4_out, num_classes, 1, padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # add in skip connections from the second pooling layer
    skip = tf.layers.conv2d_transpose(vgg_layer3_out, num_classes, 1, padding='same',
                                      kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, skip)
...
def run(epochs=5, batch_size=4):
    ...
    with tf.Session() as sess:
        ...
        train_losses = train_nn(sess,
                                ...
                                # sgd operator and loss function
                                train_op, cross_entropy_loss,
